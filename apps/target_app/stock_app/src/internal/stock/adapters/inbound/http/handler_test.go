...
// TestMapError covers the stock handler's error mapping: each domain error keeps
...
func TestMapError(t *testing.T) {
	cases := []struct {
		...
		{"not found", domain.ErrProductNotFound, fiber.StatusNotFound, "product not found"},
		{"insufficient stock", domain.ErrInsufficientStock, fiber.StatusConflict, "insufficient stock"},
		{"sku exists", domain.ErrSKUExists, fiber.StatusConflict, "sku already exists"},
		// A raw CockroachDB error (e.g. "relation \"products\" does not exist",
		// SQLSTATE 42P01) is NOT a domain error, so it must collapse to a generic
		// 500 — and never expose itself.
		{"raw db error", errors.New(`ERROR: relation "products" does not exist (SQLSTATE 42P01)`),
